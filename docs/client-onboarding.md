...
Pedir a la inmobiliaria: nombre comercial, slug deseado, nombre y email del owner, teléfono, WhatsApp, dirección, email público, logo, imagen Hero y color de marca.
...
```bash
pnpm client:onboard --name "Inmobiliaria Ejemplo" --slug "inmobiliaria-ejemplo" --owner-name "Nombre Owner" --owner-email "owner@ejemplo.com" --phone "02266123456" --whatsapp "5492266123456" --address "Calle 1 123" --public-email "hola@ejemplo.com" --primary-color "#1B1B1B" --logo "C:\\ruta\\logo.png" --hero "C:\\ruta\\hero.webp"
```

`--logo`, `--hero` y los datos de contacto son opcionales. El slug debe ser único; una colisión detiene la transacción. Si falla la operación, el script elimina el usuario invitado y los assets que haya subido.
