...
export function LocationPicker({ addressFieldNames, fallbackCoordinates, geocodeAction, initialCoordinates = null, markerColor }: LocationPickerProps) {
  ...
  function search(query: AddressQuery) {
    ...
    startSearching(async () => {
      ...
      if (!result.ok) {
        setFeedback(result.reason === "not-found" ? "No encontramos esa dirección. Ubicá la propiedad haciendo clic directamente en el mapa." : "No pudimos buscar ahora. Podés ubicar el punto directamente en el mapa.");
        return;
      }
      setCandidates(result.candidates);
      setHouseNumberNotFound(result.houseNumberNotFound);
      if (result.candidates.length === 0) {
        setFeedback("No encontramos esa dirección. Ubicá la propiedad haciendo clic directamente en el mapa.");
        return;
      }
      if (result.candidates.length === 1) selectCandidate(result.candidates[0]);
      if (result.houseNumberNotFound) setFeedback("No encontramos la altura exacta. Seleccioná la opción más cercana y ajustá el marcador en el mapa.");
    });
  }

  function selectCandidate(candidate: Candidate) {
    setCoordinates(candidate.coordinates);
    setFeedback(houseNumberNotFound ? "No encontramos la altura exacta. Seleccioná la opción más cercana y ajustá el marcador en el mapa." : candidate.hasHouseNumber ? "Ubicación seleccionada. Podés ajustar el marcador." : "Resultado aproximado sobre la calle. Podés ajustar el marcador.");
  }
  ...
  function handleStructuredSearch() {
    ...
    search({ address, city: values.city ?? values[addressFieldNames[1]], province: values.province ?? values[addressFieldNames[2]], country: values.country ?? values[addressFieldNames[3]] });
  }
  ...
  return <div ref={pickerRef} className="flex flex-col gap-3">
    ...
    <Button className="w-full sm:w-fit" disabled={isSearching} onClick={handleStructuredSearch} type="button" variant="outline"><Search aria-hidden="true" data-icon="inline-start" />{isSearching ? "Buscando…" : "Buscar esta dirección"}</Button>
