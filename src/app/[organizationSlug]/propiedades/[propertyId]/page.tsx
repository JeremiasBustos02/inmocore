...
export async function generateMetadata({
  ...
  const publicLocation = property.locationLabel;
  const publicTitle = property.title;
  const price = property.priceAmount !== null && property.currency
    ? formatPublicPrice(property.priceAmount, property.currency)
    : null;
  const title = `${propertyTypeLabel} en ${operationLabel.toLowerCase()}${property.city ? ` en ${property.city}` : ""} | ${organization.name}`;
  const propertyFacts = [
    `${propertyTypeLabel} en ${operationLabel.toLowerCase()}`,
    publicLocation || null,
    price,
    ...
    property.bathrooms !== null
      ...
      : null,
    property.totalAreaM2 !== null ? `${property.totalAreaM2} m²` : null,
  ].filter(Boolean).join(" · ");
  const editorialDescription = property.locationVisibility === "exact"
    ? property.description?.replace(/\s+/g, " ").trim()
    : null;
  const propertyDescription = [propertyFacts, editorialDescription]
    .filter(Boolean)
    .join(". ")
    .slice(0, 155);
  ...
  return {
    ...
    openGraph: {
      ...
      ...(property.images[0]
        ? {
            images: [{ url: property.images[0].url, alt: `${publicTitle} en ${property.city}` }],
          }
        : {}),
    },
...
export default async function PublicPropertyPage({ params }: PublicPropertyPageProps) {
  ...
  const catalogHref = getPublicPath(publicBasePath, "/propiedades");
  const homeCanonical = getOrganizationPublicUrl(organization);
  const catalogCanonical = getOrganizationPublicUrl(organization, "/propiedades");
  const propertyCanonical = getOrganizationPublicUrl(
    organization,
    `/propiedades/${property.id}`,
  );
  const publicTitle = property.title;
  const breadcrumbItems = homeCanonical && catalogCanonical && propertyCanonical
    ? [
        { name: "Inicio", item: homeCanonical },
        { name: "Propiedades", item: catalogCanonical },
        { name: publicTitle, item: propertyCanonical },
      ]
    : null;
  const features = [
    property.rooms !== null
  ...
  const mapsHref = property.location
    ...
    : null;
  const whatsappMessage = `Hola, quiero consultar por la propiedad ${property.propertyCode}: "${publicTitle}"${property.city ? ` en ${property.city}` : ""}.`;
  ...
  const contactDescription = "Contactanos por WhatsApp para realizar una consulta.";
        ...
        <article className="mx-auto w-full max-w-[1440px] px-5 py-10 sm:px-8 sm:py-14 lg:px-8 lg:py-16">
          <nav aria-label="Migas de pan" className="text-sm text-muted-foreground">
            <ol className="flex flex-wrap items-center gap-2">
              <li><Link className="public-link" href={publicBasePath || "/"}>Inicio</Link></li>
              <li aria-hidden="true">/</li>
              <li><Link className="public-link" href={catalogHref}>Propiedades</Link></li>
              <li aria-hidden="true">/</li>
              <li aria-current="page" className="max-w-[min(60vw,32rem)] truncate font-medium text-foreground">{publicTitle}</li>
            </ol>
          </nav>
          {breadcrumbItems ? (
            <script
          ...
          <div className="mt-7">
            <PropertyGallery city={property.city} images={property.images} propertyTitle={publicTitle} />
          </div>
